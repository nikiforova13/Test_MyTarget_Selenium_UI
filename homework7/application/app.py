...
def create_user():
    ...
    if user_name not in app_data:
        ...
        return (
            jsonify(f"User_name {user_name} already exists: id: {app_data[user_name]}"),
            400,
        )
...
def get_user_id_by_name(name):
    if user_id := app_data.get(name):
        ...
        age = None
        try:
            # Обращаемся в сторонний сервер, который выдает ответы.
        ...
        try:
            response = requests.get(
                f"http://{surname_host}:{surname_port}/get_surname/{name}"
            )
            if response.status_code == 200:
                surname = response.json()
        except Exception as err:
            print(f"Unable to get surname from external system:\n{err}")
        data = {"user_id": user_id, "age": age, "surname": surname}
