...
class AgeStubHandleRequests(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # запрос
        location = self.path.split("/")  # все пути сами делаем
        if (
            len(location) == 3
            and location[1] == "get_age"
            and isinstance(location[2], str)
        ):
            self._set_headers(200)  # Отдаем ответ
            self.wfile.write(
                json.dumps(random.randint(18, 105)).encode()
            )  # Представляем в json
        else:
